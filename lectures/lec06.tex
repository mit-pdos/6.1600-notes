...
We want $F(\pk, \cdot)$ and $I(\sk, \cdot)$ to be mutually inverse
permutations of $\cal X$.  Formally, for all $\lambda \in \N$,
$(\sk, \pk) \gets \Gen(1^\lambda)$, and $x, y \in \calX$:
\[ I(\sk, F(\pk, x)) = x \qquad\text{and}\qquad F(\pk, I(\sk, y)) = y. \]
\marginnote{The second identity follows from the first when $\cal X$ is
finite: $I \circ F = \mathrm{id}$ forces $F$ to be injective, hence a
bijection on a finite set, so $I = F^{-1}$ and the two compose to the
identity in either order.  We state both because the signature
construction below needs the second, and because for a trapdoor one-way
\emph{function} --- where $F$ need not be onto --- the second can fail.}
...
\paragraph{Correctness.}
For all $\lambda \in \N$, $(\sk, \pk) \gets \Gen(1^\lambda)$ and
$m \in \zo^*$, the signature is $\sigma = I(\sk, H(m))$, and
verification recomputes $H(m)$ and checks
$F(\pk, \sigma) = F(\pk, I(\sk, H(m))) = H(m)$,
which holds by correctness of the trapdoor one-way permutation.
Hence, $\Ver(\pk, m, \Sign(\sk, m)) = 1$.
  ...
  \begin{itemize}
    \item Sample two random $\lambda$-bit primes $p$ and $q$
      such that $p \equiv q \equiv 4 \pmod 5$.\marginnote{%
        ...
        Using the stronger condition $p \equiv q \equiv 4 \pmod 5$ here simplifies 
...
\marginnote{Two natural numbers are \emph{relatively prime} if they share no prime factors.}
...
a value $y \getsr \Z^*_N$ and must find a value $x$ such that $x^5 = y \bmod N$.
Or, put another way, the attacker's task is essentially the following:
\begin{itemize}
  \item \textbf{Given:} A polynomial $p(X) \deq X^5 - y \in \Z_N[X]$,
                        for $y \getsr \Z*_N$.
  \item \textbf{Find:}  A value $x \in \Z^*_N$ such that $p(x) = 0 \in \Z^*_N$.
\end{itemize}

So the attacker must find the root of a polynomial modulo a composite integer~$N$.
\marginnote{We work in $\Z_N^*$ rather than $\Z_N$ because
$x \mapsto x^5$ is a permutation only on the elements of $\Z_N^*$.  If $y$ is a
non-unit---say $y = p$---the equation $x^5 = y \bmod N$ has no
solution at all, so ``find a root'' would not be a well-posed task.
This matters only for a $\approx 2^{-\lambda+1}$ fraction of $\Z_N$,
and an attacker who stumbled on such a $y$ could recover $p$ from
$\gcd(y, N)$ and factor $N$ outright.}
...
%}
Since the best known algorithm, the general number field sieve, runs in
time
\[ \exp\Big( \big(\sqrt[3]{64/9} + o(1)\big)
   (\ln N)^{1/3} (\ln \ln N)^{2/3} \Big), \]
this approach is infeasible at present without knowing the
factorization of $N$.\marginnote{For a $2048$-bit modulus $N$ (that is,
$\lambda = 1024$-bit primes) this is roughly $2^{117}$ operations. The
largest RSA modulus factored to date is $829$ bits, which took about
$2700$ core-years in 2020. Note that $\log N = 2\lambda$, so doubling
the prime size more than doubles the security level: a $4096$-bit
modulus costs about $2^{157}$.}
...
in $\Z_N = \{0, 1, 2, 3, \dots, N\}$ that are relatively prime to $N$.
...
\begin{proof}
  Since $p \equiv q \equiv 4 \pmod 5$, we have $p - 1 \equiv q - 1 \equiv
  3 \pmod 5$, and so
  \[ \phi(N) = (p-1)(q-1) \equiv 3 \cdot 3 = 9 \equiv 4 \pmod 5. \]
  Hence $\phi(N) - 4 \equiv 0 \pmod 5$, so $\frac{\phi(N)-4}{5}$ is an
  integer and $d$ is well defined.  Then
  \[ 5d = \big(\phi(N) - 4\big) + 5 = \phi(N) + 1 \equiv 1 \pmod{\phi(N)}. \]
\end{proof}
