...
adversary MACs a message that is gibberish, they
...
definition of \textquote{non-gibberish} we define.
...
%(\cref{def:crhf})
and we define the MAC on message space $\zo^*$ as:
\[ \MAC(k,m) \deq F(k, H(m)).\]
...
If we use the same key $k$, an adversary can mount a length-extension attack.
Writing $E(x) = F(k,x)$, an $n$-block message costs $n+1$ applications of $E$.
The adversary queries the one-block message $m_1$, receiving $t = E(E(m_1))$, and
then queries the one-block message whose contents are the bit string $t$,
receiving $t' = E(E(t))$. But
$\mathsf{MAC}(k, m_1 \| 0 \| 0) = E(E(E(E(m_1)))) = E(E(t)) = t'$,
so $t'$ is a valid tag on a message the adversary never queried. Two zero blocks
are needed because the second query contributes two applications of $E$: one for
its message block and one for its final step. A zero block is free to append
since $0 \oplus t_{i-1} = t_{i-1}$, so it passes the chaining value through and
adds one more $E$.
}
...
fixed-size chunks $m_1, m_2, \ldots, m_{l}$.\marginnote{In practice,
these fixed-size chunks are going to be 128 or 256 bits long.}  Then,
the hash of that message is defined as:

$$
H(k, m_1 || m_2 || \ldots || m_{l}) = (m_1 + m_2 k + m_3 k^2 +
  \ldots + m_{l} k^{l-1} + k^{l}) \mod p
$$

\marginnote{The final monic term $k^l$ is essential, even though it
involves no message block. Without it, a message and the same message
with trailing zero chunks appended hash to the same value for
\emph{every} key: the added chunks contribute nothing to the sum.
The term $k^l$ makes the map from messages to polynomials injective,
since the degree records the number of chunks and the remaining
coefficients record their contents.}
...
$$
(m_1 - m'_1) + (m_2 - m'_2) k + \ldots + (m_{l} - m'_{l}) k^{l-1}
  + (k^{l} - k^{l'}) = 0
$$

which is another way of saying that $k$ is a root of that polynomial.
That polynomial is not identically zero.  If the two messages have the
same number of chunks $l = l'$, the two monic terms cancel and some
coefficient $m_i - m'_i$ is non-zero because $m \neq m'$.  If instead
$l < l'$, the term $-k^{l'}$ survives and the polynomial has degree
exactly $l'$.  Either way we have a non-zero polynomial of degree at
most $\max(l, l')$, so it has at most that many roots, but there are
$p \approx 2^n$ possible choices for $k$.  The probability that our
randomly-chosen $k$ happens to be one of those roots is therefore at
most $\frac{\max(l,l')}{p}$, which is negligible.
...
\begin{itemize}[noitemsep]
  ...
  \item Compute $M(v) \gets m_1 v + m_2 v^2 + m_3 v^3 + \cdots
        + m_{\ell} v^{\ell} + v^{\ell+1} \in \Z_p$. \\
        \marginnote{Essentially we are viewing the blocks of the message 
        $m$ as coefficients of a degree-$(\ell+1)$
        polynomial $M(\cdot)$. We then evaluate this polynomial 
        at the secret point $v$ determined by the MAC key.
        The trailing monic term $v^{\ell+1}$ carries no message block;
        its job is to record the message length in the degree.}
...
\paragraph{Why the trailing monomial?}
Suppose we had defined $M(v) = m_1 v + \dots + m_{\ell} v^{\ell}$, without
the final $v^{\ell+1}$ term.  Then appending a zero block to a message would
leave the polynomial unchanged, and the scheme would be forgeable after a
single query.  An adversary asks for a tag on the one-block message
$m = (m_1)$ and receives $t = (r, z)$ with $z = F(k,r) + m_1 v$.  The message
$m^* = (m_1, 0) \neq m$ has
\[
  M^*(v) = m_1 v + 0 \cdot v^2 = m_1 v = M(v),
\]
so $\MACVerify(k, m^*, t)$ accepts the very same tag, with probability $1$.
More generally $m$ and $m \concat 0^j$ would collide for every $j$.
The trailing monic term rules this out by making the map from messages to
polynomials injective: the degree of $M$ determines $\ell$, and the
remaining coefficients determine $m_1, \dots, m_{\ell}$.
...
\begin{itemize}
  ...
  \item Next, we show that as long as the $\MACSign$ algorithm never
        ...
        are independent random values that completely hide the values $M(v)$.
    ...
    There are two cases:
        \begin{itemize}
          ...
          \item Alternatively, the forger could use a random nonce $r^*$
            ...
            MAC queries.
            In this case, we have the following relations, where $M$ is
            the polynomial of the message $m$ that the adversary queried
            of the challenger:
            \begin{align*}
              F(k, r) &= z - M(v)\\
              F(k, r) &= z^* - M^*(v)\\
            ...
            \end{align*}
            Since $m \neq m^*$ and the message encoding is injective,
            $M(\cdot)$ and $M^*(\cdot)$ are distinct polynomials, so
            $\big(M(\cdot) - M^*(\cdot)\big) + (z^* - z)$ is not identically
            zero.\marginnote{The added constant $z^* - z$ cannot cancel
            $M(\cdot) - M^*(\cdot)$, because neither $M$ nor $M^*$ has a
            constant term, so their difference is never a non-zero constant.}
            It is a non-zero polynomial of degree at most $L+1$, so it has at
            most $L+1$ roots in $\Z_p$.
            Since the adversary's view is independent of the evaluation
            point $v \in \Z_p$, the probability that the adversary's 
            forgery is valid is at most $(L+1)/p$.
        \end{itemize}
...
Note that nonce reuse is catastrophic. Say two distinct messages $m \neq m'$
that produce $(r, z)$ and $(r, z')$, where $z = F(k,r) + M(v)$ and
$z' = F(k,r) + M'(v)$. The adversary is able to discover the secret value $v$, and then forge any message.


% \paragraph{Why the universal hash function may have a constant term.}
% In the universal-hash game there is no adversary-supplied offset.  The
% adversary picks $m \neq m'$, and we need only that
% $H(\cdot, m) - H(\cdot, m')$ is a non-zero polynomial: with a constant term
% that difference is
% \[
%   (m_1 - m'_1) + (m_2 - m'_2) k + \dots,
% \]
% which is non-zero as soon as the messages differ.  The root-counting argument
% then applies unchanged.  So the constant term in the definition of $H$ above
% does no harm.
% 
% \paragraph{Why the MAC's message polynomial must not have one.}
% In the MAC, by contrast, the adversary chooses the second component $z^*$ of
% the tag freely, and $\MACVerify$ accepts when $z^* - F(k, r^*) = M^*(v)$.
% Choosing $z^*$ is therefore the same as adding an arbitrary constant to the
% message polynomial, and a constant term in $M$ gives the adversary something
% that this constant can cancel.
% 
% Concretely, suppose we had defined $M(v) = m_1 + m_2 v + \dots + m_{\ell}
% v^{\ell-1}$, in the style of the universal hash function above.  An adversary
% queries the two-block message $m = (m_1, m_2)$ and receives $t = (r, z)$ with
% $z = F(k, r) + M(v)$.  It then outputs the forgery
% \[
%   m^* = (m_1 + 1,\; m_2), \qquad t^* = (r,\; z + 1),
% \]
% reusing the queried nonce.  Verification computes
% \[
%   z^* - F(k, r) = \big(F(k,r) + M(v) + 1\big) - F(k,r) = M(v) + 1 = M^*(v),
% \]
% so the tag is accepted.  The forgery succeeds with probability $1$, after a
% single query, and the adversary never learns anything about
% $v$.\marginnote{Deployed constructions agree.  \textsf{GHASH}, the universal
% hash inside AES-GCM, evaluates the powers $H^1, \dots, H^m$ and never $H^0$;
% Poly1305 likewise starts at $r^1$ and adds its mask separately.  Neither has a
% constant term.}
% 
% This is the same failure as the missing trailing monomial, in a different
% disguise: both are ways for $M(\cdot) - M^*(\cdot)$ to be cancelled out
% entirely, one by appending zero blocks and one by the adversary's free choice
% of constant.  The trailing monic term $v^{\ell+1}$ handles the message length;
% starting the polynomial at $m_1 v$ rather than $m_1$ handles the offset.
% Together they are exactly what the security argument needs when it observes
% that $\big(M(\cdot) - M^*(\cdot)\big) + (z^* - z)$ is not identically zero.

% After the lecture Adem asked me if in Carter-Wegman we could set $$v$$ to be $$k$$, right now it is defined as $$v = F(k, 0)$$.
% 
% I'm not aware of an efficient attack on the $$v = k$$ variant when instantiated with AES. However, you'd be relying on a key-dependent-input property AES was never analyzed for, in exchange for saving one block cipher call. You are now exposing many $$F(k, r) + M(k)$$ pairs, as opposed to $$F(k, r) + M(v)$$ pairs, where $$v$$ is unrelated to $$k$$. That trade is why the $$v = F(k, 0)$$ line is there.
% 
% Further, a repeated nonce lets the adversary recover the evaluation point $$v$$.  If you set $$v = k$$, you have lost your master key. So the damage is catastrophic, and not worth the saved block cipher call.
