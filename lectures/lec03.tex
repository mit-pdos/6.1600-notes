...
\begin{table}[htpb]
    ...
    $2^{95}$ & hashes/yr on the Bitcoin network\\
		...
		$2^{140}$ & hashes required to use one year of the sun's total energy output \\
	\end{tabular}
\end{table}
...
Let $m''$ be any suffix the attacker wishes to append,
padded to a whole number of blocks. Given $m$ and $t$, the attacker
computes
\begin{enumerate}
...
\begin{figure}[htpb]
  ...
  \caption{Sketch of the extension attack on the Merkle-Damg\aa{}rd
    ...
    to compute the hash of $m_1 || m_2 || \ldots || m_n || \textsf{ToBlock}(n) || m''$.}
