...
\marginnote{For most cryptosystems, there is a tradeoff between the attacker's running time and success probability.  For example, an attacker running in time $T$ can find a collision in a hash function with $n$-bit output with probability $T^2/2^n$.  So, as the attack runs for more time, it has a better chance of finding a collision.  }
...
\marginnote{In practice, standard hash functions have their own limits on the
...
\paragraph{Why the length block is necessary.}
Suppose we delete the final step $b \gets H_\text{small}(\mathsf{ToBlock}(n))$
from \Cref{fig:md}, and call the resulting function $H^-$.
Collisions between two messages with the \emph{same} number of blocks still
...
So any attack must use messages of \emph{different} lengths, and the job of
...
resistance of $H_\text{small}$ does not \emph{imply} that of $H^-$.
We do this by constructing an $H_\text{small}$ that is collision resistant
and yet makes $H^-$ trivially insecure.

\begin{const}[A collision-resistant $H_\text{small}$ that breaks $H^-$]
  \label{const:md-no-length}
  Let $G\colon \bin^{2\lambda} \rightarrow \bin^{\lambda-1}$ be any
  collision-resistant function, and define
  $H_\text{small}\colon \bin^{2\lambda} \rightarrow \bin^{\lambda}$ by
  \[
    H_\text{small}(b, m) =
    \begin{cases}
      0^{\lambda} & \text{if } b \concat m = 0^{2\lambda},\\[2pt]
      1 \concat G(b, m) & \text{otherwise.}
    \end{cases}
  \]
\end{const}
...
By construction $H_\text{small}$ has a \emph{fixed point at the initialization
value}:\marginnote{This is the only property of $H_\text{small}$ the attack
uses. Any $H_\text{small}$ with $H_\text{small}(0^\lambda, m^*) = 0^\lambda$
for some block $m^*$ is equally fatal; prepending copies of $m^*$ then leaves
the hash unchanged.}
\[
...
iteration that does nothing, and so for \emph{every} message $m$,
\[
  H^-(0^{\lambda} \concat m) = H^-(m).
\]

Taking $n = 1$ and $n = 2$ gives an explicit collision.
With $\lambda = 32$ and $G$ instantiated by truncating \textsf{SHA-256}
to its first $31$ bits, the two messages
\[
  M = \ttt{41414141}
  \qquad\text{and}\qquad
  M' = \ttt{00000000} \concat \ttt{41414141}
\]
both satisfy $H^-(M) = H^-(M') = \ttt{d740d7ef}$.
Note that $M$ and $M'$ differ in content, not merely in length.
The same trick works at any size: for $n = 2$ versus $n = 3$,
\[
  \ttt{41414141} \concat \ttt{42424242}
  \qquad\text{and}\qquad
  \ttt{00000000} \concat \ttt{41414141} \concat \ttt{42424242}
\]
both hash to \ttt{d314a585}. Degenerately, every all-zeros message
...
value \ttt{d740d7ef} but distinct second arguments
$\mathsf{ToBlock}(1)$ and $\mathsf{ToBlock}(2)$, so their hashes
\ttt{ca06ae43} and \ttt{dae0f489} could only coincide if
...
(We will see the formal definition in the lecture on message authentication codes.)
%cref{lec:mac}.)

\paragraph{Bad idea.}
Poorly designed software uses $\MAC(k, m) = H(k \| m)$ as a very simple
construction of a MAC.
If $H$ is built with the Merkle-Damg\aa{}rd construction, this has an easy
attack: an adversary who sees a single valid tag can forge a tag on a
\emph{longer} message, without ever learning the key~$k$.

The reason is that a Merkle-Damg\aa{}rd hash \emph{is} its own chaining
variable. Write $n$ for the number of blocks in $k \| m$, and let
\[
  t = \MAC(k, m) = H(k \| m).
\]
By \Cref{fig:md}, $t$ is exactly the value of the chaining variable $b$ after
...
keeps iterating $H_\text{small}$ is simply resuming the hash computation where
the honest party stopped.

\paragraph{The attack.}
Let $m''$ be any suffix the attacker wishes to append,\marginnote{In
\Cref{fig:merkle-damgard-extension} this attacker-chosen suffix is labelled
$x$.} padded to a whole number of blocks. Given $m$ and $t$, the attacker
computes
\begin{enumerate}
  \item $b \gets H_\text{small}(t, m'')$, and
  \item $b \gets H_\text{small}(b, \mathsf{ToBlock}(n+2))$,
\end{enumerate}
and outputs $b$. This is a valid tag on a message the attacker never had the
key to authenticate.

\paragraph{Which message was forged?}
Here is the subtle point. The attacker resumed the chain \emph{after} the
...
$m \| m''$. In practice this is rarely an obstacle---the glue is a short run of
bytes, and many formats (query strings, log lines, serialized records) will
happily ignore or tolerate it.
