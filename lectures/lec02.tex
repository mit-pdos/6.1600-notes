...
Typically, a computer system performs two or more steps before processing
a request:
\begin{enumerate}
...
\begin{itemize}
  ...
  \item \emph{Active attack.} 
        ...
        signatures, aim to protect against active attacks.
...
\begin{enumerate}
  ...
  \item The client computes an authentication ``tag'' $t \gets F(k, c)$, where $F(\cdot, c)$ is hard to compute without knowing $k$.
        ...
        which we will talk more specifically about in a later lecture.) % \cref{lec:mac}.)
...
to give her the one-time code by pretending to be
...
scheme based on digital signatures, which we will discuss in a lecture on signatures.
% \cref{sec:sig}.
