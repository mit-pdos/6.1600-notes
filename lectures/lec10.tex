...
\begin{enumerate}
  ...
  \item A much better attack is the Number Field Sieve, which is able to compute $x$ in (roughly) time $\exp((\log p)^{1/3} (\log \log p)^{2/3})$---sub-exponential time!
    \marginnote{An attack that runs in time $\sqrt p$ runs in time $\exp(\frac 12 \log p)$. 
    In contrast, the Number Field Sieve runs in time roughly $\exp(\log^{1/3} p)$. This is much much much faster than the $\sqrt p$-time algorithms,
