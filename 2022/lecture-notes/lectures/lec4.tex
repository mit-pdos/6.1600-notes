...
\begin{definition}[MAC Security: Existentially unforgeability against adaptive chosen message attacks]
...
game with at most negligible probability:
  \begin{itemize}[noitemsep]
    ...
    \item For $i = 1, 2, \dots$ (polynomially many times)
      \begin{itemize}
        \item The adversary $\calA$ sends any message $m_i \in \calM$ 
              to the challenger 
...
With this 128-bit PRF, we can build a MAC for 128-bit messages as described above: $\mathsf{MAC}(k, m) = \mathsf{AES}_k(m)$. 
However, since AES takes only 128 bits as input, using AES
directly, we can only generate MACs for 128-bit messages. 
