...
Which of these are "good" and which are "bad"? Many sites would let you have \ttt{PaSsW0rd1!} but would not allow \ttt{purple-student-hat}. However, what really matters is the way that the adversary will guess passwords. And most likely, they will guess the most common passwords first.

%% make graph of spikey entropy, decaying entropy when sorted by popularity
...
%% chart of the top 20 passwords or something? I think that is interesting

So what we really want is for our password to have high entropy. How do we get there?

\begin{itemize}
	\item Require longer password? If someone tries to use \ttt{abc123} as a password but it's not long enough, they might use \ttt{abc123456}---but this doesn't really add any real entropy. There are predictable ways to lengthen passwords.
